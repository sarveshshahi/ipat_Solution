import React from 'react';
import Link from 'next/link';

function Service() {    
    return (
        <>

        <section id="service-menu" className="service-twelve-area light-area bg-sub-dark pt-130 pb-130">
            <div className="container">
                <div className="section-header__flex mb-50">
                    <div>
                        <h6 className="text-white">We Provide the Best Service</h6>
                        <h2 className="wow splt-txt text-white" data-splitting>Your Digital Transformation</h2>
                    </div>
                    <div>
                        <p className="text-white wow fadeInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
                            By bringing together the right teams, we uncover the best paths forward, elevating your
                            vision of success through our
                            digital agency services.</p>
                    </div>
                </div>
                <div className="row g-0">
                    <div className="col-lg-4">
                        <div className="service-twelve__image">
                            <img src="assets/images/service/service-twelve-image.jpg" alt="image" />
                        </div>
                    </div>
                    <div className="col-lg-4">
                        <div className="service-twelve__item">
                            <h3 className="mb-10">Business Intelligence</h3>
                            <p>We empower businesses to generate innovative product ideas through intelligent insights
                                and data-driven strategies. By
                                combining market intelligence</p>
                            <div className="list mt-20">
                                <ul>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Optimize resource allocation
                                    </li>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Assessment Report Analysis
                                    </li>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Team Building Leadership
                                    </li>
                                </ul>
                                <ul>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Growth Method Analysis
                                    </li>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Business Development
                                    </li>
                                    <li>
                                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M13.0304 1.73124L5.33652 9.35997C5.24203 9.455 5.12944 9.53046 5.00528 9.58197C4.88112 9.63348 4.74788 9.66 4.61329 9.66C4.4787 9.66 4.34546 9.63348 4.2213 9.58197C4.09714 9.53046 3.98455 9.455 3.89006 9.35997L0.299571 5.7999C0.107759 5.60971 -2.02107e-09 5.35176 0 5.08279C2.02106e-09 4.81383 0.107759 4.55588 0.299571 4.36569C0.491383 4.17551 0.751536 4.06866 1.0228 4.06866C1.29406 4.06866 1.55421 4.17551 1.74603 4.36569L4.61329 7.20358L11.584 0.297033C11.7758 0.106846 12.0359 -2.00395e-09 12.3072 0C12.5785 2.00395e-09 12.8386 0.106846 13.0304 0.297033C13.2222 0.48722 13.33 0.745169 13.33 1.01413C13.33 1.2831 13.2222 1.54105 13.0304 1.73124Z"
                                                fill="#292C2B" />
                                        </svg>
                                        Growth Method Analysis
                                    </li>
                                </ul>
                            </div>
                            <Link href="/page-about" className="btn-one dark-mode px-5 rounded-0 mt-40 text-white" data-splitting data-text="Read More">Read More</Link>
                        </div>
                    </div>
                    <div className="col-lg-4">
                        <div className="service-twelve__item-dark">
                            <h3 className="text-white mb-15">Service List</h3>
                            <div className="list">
                                <ul>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Financial Planning</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Performance Analysis</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Investment Advisory</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Business Intelligence</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Manage Investment</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Tax Planning</Link></li>
                                </ul>
                                <ul>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Business Innovations</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Collaborative Approach</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Technology Integration</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Custom Solutions</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Transforming Futures</Link></li>
                                    <li><Link href="/page-service-details"><i className="fa-light fa-arrow-right-long" /> Guiding Excellence</Link></li>
                                </ul>
                            </div>
                            <div className="rectangle">
                                <div className="item wow"></div>
                                <div className="item-dark wow"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        </>
    );
}
export default Service